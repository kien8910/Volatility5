"""Stock-level q90/q95 original-volatility spike classification."""
...
def plan_tasks(
    ...
    *,
    ...
    for variant in variants:
        ...
        for quantile in config["targets"]["ticker_spike_quantiles"]:
            for threshold_mode in profile["spike_threshold_modes"]:
                for input_variant in input_variants:
                    for model in profile["spike_models"]:
                        for fold in profile["folds"]:
                            for seed in profile["seeds"]:
                                if not fold_task_supported(
                                    ...
                                    continue
                                target = (
                                    f"volatility_spike_q{int(float(quantile) * 100)}_"
                                    f"{threshold_mode}"
                                )
                                payload = {
                                    **variant,
...
def run_task(
    ...
    y_train, y_validation, y_test, thresholds = stock_spike_labels(
        data,
        float(task.config["quantile"]),
        str(task.config["threshold_mode"]),
